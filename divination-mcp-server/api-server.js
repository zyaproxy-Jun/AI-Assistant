    ...
    if (req.url.startsWith('/api/') && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                // 统一的 divination 端点
                if (endpoint === 'divination') {
                    toolName = data.tool;
                    result = await callMCPTool(data.tool, data.args);
                } else {
                    // 兼容单独的端点
                    switch (endpoint) {
                        case 'tarot':
                        ...
                        case 'bazi':
                            toolName = 'bazi_analysis';
                            result = await callMCPTool('bazi_analysis', data);
                            break;
                        
                        case 'iching':
                            toolName = 'iching_divination';
...
async function start() {
    ...
    try {
        ...
        server.listen(PORT, () => {
            ...
            console.log('   POST /api/divination  - Universal divination endpoint');
            console.log('   POST /api/tarot       - Tarot reading');
            console.log('   POST /api/ziwei       - Ziwei astrology');
